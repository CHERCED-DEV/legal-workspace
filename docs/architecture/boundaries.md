...
**Revisiones.** `CaseRevision` monotónica por Case, con `seq == revision`. Concurrencia **optimista** sin locking pesimista: mismatch de `expected_revision` ⇒ rechazo del commit, Proposal preservada en `PRESERVED_FOR_RECONCILIATION` y condición `REVISION_CHANGED {expected, current, preserved_proposal_id}`. Nunca sobrescritura silenciosa, nunca descarte del trabajo.
