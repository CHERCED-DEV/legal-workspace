...
1. **Case Event Log (canónico).** Un solo log **append-only, hash-chained**, por Case, que **unifica** el evento de dominio/aplicación y el evento de auditoría. Cada evento: `event_id, case_id, event_seq (monotónico, +1 en TODO evento), case_revision (NULL si el evento no muta el estado epistémico canónico — **ENMIENDA AC-02 aprobada**, supersede §16.16), operation, principal_id / principal_type / principal_role, provenance_kind, payload` (cambio completo o resumen estructurado suficiente para reconstrucción), `methodology_version` (si aplica), `model_id` (si el actor es AI), `knowledge_pack_versions` (si aplica), `timestamp, prev_hash, hash`. **Justificación de la unificación:** un evento de dominio que ya porta actor, payload y hash encadenado ES el registro de auditoría; mantener dos streams (eventos + auditoría) duplicaría el mismo contenido con riesgo de divergencia entre ambos. Esto resuelve la ambigüedad detectada en v0.1.1 E.1.
   ...
   **Momento de emisión en el ciclo de propuesta (regla normativa, ver ADR-005):** `propose_facts` emite `FactsProposed`; `ReviewProposal(approve|reject)` emite `ProposalReviewed(...)` en el acto mismo de revisión y avanza únicamente `event_seq`: **NO avanza la `CaseRevision`** (`case_revision` NULL) — **ENMIENDA AC-02 aprobada** (supersede §16.16): una revisión aún no commiteada no añade hechos, evidencia ni links, luego el expediente sabe lo mismo antes y después. En consecuencia, la autorización congela como `expected_case_revision` **la revisión vigente del Case en el momento del acto de revisión** (no `base_case_revision`), con lo que desaparece la circularidad de la definición anterior. En ese mismo acto nace la HumanAuthorization; **`commit_reviewed_facts` emite `FactsCommitted` y es el único de los dos actos que avanza la `CaseRevision`**: si `FactsProposed` deja el Case en la revisión N, `ProposalReviewed` lo deja en **N** y `FactsCommitted` lo deja en **N+1** — dos eventos en una sola revisión de conocimiento, coherente con la biyección del invariante 5, que se expresa sobre `event_seq`. Texto superado: «y avanza la CaseRevision —en ese acto se crea la HumanAuthorization, que congela como `expected_case_revision` la revisión resultante de ese acto de revisión, no la revisión contra la que se creó la Proposal—; `commit_reviewed_facts` emite `FactsCommitted` y avanza la CaseRevision de nuevo. Son dos eventos en dos revisiones distintas.»
...
5. **Biyección mutación↔evento.** **Mutación** = cambio de estado canónico registrado, **no** invocación de tool. Una sola invocación puede producir de 1 a n mutaciones, y por tanto de 1 a n eventos del Case Event Log. **ENMIENDA AC-02 aprobada** (supersede §16.16): la biyección se expresa sobre **`event_seq`** —que avanza en todo evento— y `case_revision` es una **subsecuencia** que avanza solo en los eventos que mutan el estado epistémico canónico. El invariante es: **toda mutación produce exactamente un evento, y todo evento corresponde a exactamente una mutación** — biyección mutación↔evento, no invocación↔evento. Cada evento porta su `event_seq`, y `case_revision` únicamente cuando muta el estado epistémico canónico (NULL en los demás). La formulación previa "n mutaciones == n eventos" queda como abreviatura de esta biyección y solo es correcta bajo esta definición de mutación (supersede §16.11 del addendum v0.3: enunciado sin definir mutación → biyección con `mutación = cambio de estado canónico registrado`). Formulación superada: «avanzando la CaseRevision en n (caso real del slice: una incorporación que además invalida análisis deja `EvidenceIncorporated` + `ArtifactMarkedStale`, dos eventos y dos revisiones) … cada evento porta `seq == CaseRevision` resultante.»
...
2. **Property test de la biyección mutación↔evento:** para toda secuencia de commands aceptados, cada mutación de estado canónico registrada tiene exactamente un evento del Case Event Log y cada evento corresponde exactamente a una mutación, con **`event_seq` contiguos** y `case_revision` como subsecuencia de los eventos canónicos, reportada por las tools (**ENMIENDA AC-02 aprobada**: la identidad `seq == case_revision` queda superada, supersede §16.16). El test verifica **la biyección, no el conteo de invocaciones**: una invocación que produce n mutaciones debe dejar n eventos —avanzando `event_seq` en n— y avanzar `case_revision` solo en aquellos que mutan el estado epistémico canónico (invariante 5); contar llamadas a tools daría un falso fallo en ese caso legítimo.
...
- **ADR-005** (HumanAuthorization): `commit_reviewed_facts` valida `expected_case_revision` de la autorización contra la CaseRevision vigente; el mismo mecanismo de conflicto de este ADR (rechazo + preservación + `REVISION_CHANGED`) protege ese commit sensible, y su éxito produce el evento `FactsCommitted`. El ciclo de propuesta consume **una sola revisión de conocimiento**: `ReviewProposal(approve)` emite `ProposalReviewed(approved)` y avanza **solo `event_seq`**, dejando `case_revision` NULL (**ENMIENDA AC-02 aprobada**, supersede §16.16) —la autorización nace en ese acto y congela como `expected_case_revision` **la revisión vigente del Case en el momento del acto de revisión**, "la revisión que la profesional tenía a la vista al aprobar", y **no** `base_case_revision`: `FactsProposed` y `ArtifactRegistered` ya avanzaron el contador—, y **solo el commit** emite `FactsCommitted` y avanza la revisión: si `FactsProposed` deja el Case en N, `ProposalReviewed` lo deja en **N** y `FactsCommitted` lo deja en **N+1**.
