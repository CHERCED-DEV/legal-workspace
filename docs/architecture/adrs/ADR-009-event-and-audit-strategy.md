...
Cada evento registra ambos; `case_revision` es nulo en los eventos que no mutan conocimiento. La consecuencia operativa central: **`ReviewProposal` avanza `event_seq` y no `case_revision`**, y `HumanAuthorization.expected_case_revision` pasa a ser la revisión **vigente del Case en el momento del acto de revisión** —no `base_case_revision`—, no la resultante del propio acto de revisión.
