...
1. **RESUELTA — Aritmética de revisiones (enmienda AC-02, aprobada).** Los dueños aprobaron separar `event_seq` de `case_revision`: `ProposalReviewed` avanza `event_seq` y lleva `case_revision` **nula**; `expected_case_revision` es la revisión **vigente del Case en el momento del acto de revisión** —no `base_case_revision`—, con lo que desaparece la definición circular anterior. ADR-004 y ADR-005 quedan enmendados (supersedes §16.16 y §16.19). El modelo anterior —todo evento avanzaba la revisión— queda superado.
