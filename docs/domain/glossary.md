...
`propose_facts` registra la Proposal `P-3`: el evento `FactsProposed` deja el Case en la revisión **14**. La profesional revisa y aprueba: `ReviewProposal(approve)` emite `ProposalReviewed` y deja el Case en **15**; en ese mismo acto se crea la `HumanAuthorization` con `expected_case_revision = 15` — la revisión que ella tenía a la vista al aprobar (addendum B.2). Antes de que se ejecute el commit se incorpora un documento nuevo: el Case pasa a **16**. Al intentar el commit con `expected_revision = 15`, el Core lo rechaza y emite `REVISION_CHANGED {expected: 15, current: 16, preserved_proposal_id: P-3}`; `P-3` queda en `PRESERVED_FOR_RECONCILIATION` y aparece en `get_case_context(pending)`. Su trabajo de revisión no se perdió: se reconcilia contra la revisión 16.
