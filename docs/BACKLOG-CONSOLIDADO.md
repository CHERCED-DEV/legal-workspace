...
| **6** | **Una prueba capaz de fallar.** Existen 678 líneas de banco y seis fixtures que ninguna evaluación mencionó | G22 · `PM-5.1-BANCO` | La regla de composición · M-9 · M-11 · M-12 | **A medias desde el 2026-09-05.** El instrumento **ya puede fallar**: `medir.py` tenía un veredicto de un solo estado —«intacta» = cero fabricaciones— y **un run vacío tiene cero por construcción**; ahora son tres estados, el código de salida es el veredicto, y avisa si la ficha del caso está invalidada. Con 8 pruebas y dos mutantes. **Lo que sigue faltando es el material**: el instrumento puede fallar, y no hay sobre qué correrlo (fila 0.8) |
