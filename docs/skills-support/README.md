...
- **CORRECCIÓN DE 2026-09-05 — esta línea decía que la ruta vigente era `plugins/despacho/skills/fact-builder/`, y no lo es.** Esa carpeta **se borró el 2026-08-26**, un día después de la fecha de referencia de este corpus, en el commit que aplicó la crítica al arnés: **`H-09 fact-builder pasa a llamarse hechos-con-prueba`**. La ruta vigente es **`plugins/despacho/skills/hechos-con-prueba/`**, y el plugin tiene hoy **once** métodos, no uno.
- **Las menciones históricas a `fact-builder` se dejan como están, aquí y en los demás documentos del repositorio (55 en total).** Son registro de lo que se decidió con la información de entonces, y reescribirlas lo falsearía. Lo que se corrige es **la única línea que afirmaba un hecho sobre el presente**.
