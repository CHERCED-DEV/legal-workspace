...
| **Conexión** | Solo para instalar y para trabajar. El plugin son dieciséis métodos de texto más una carpeta de programas |
| **Nada más** | No hay servidor, ni base de datos, ni clave que pedir, ni nada que pagar aparte de la suscripción de ella |

**No hace falta** Git ni saber programar. **Python es opcional y hace la diferencia:** sin él, catorce de los dieciséis comandos funcionan igual —el modelo hace a mano el trabajo mecánico, más lento y gastando mucha más lectura—, y **los dos que necesitan oír una grabación, `/transcribir-audio` y `/genoma-de-voz`, no lo fingen: dicen que no pueden**; con él, la oficina de programas del plugin prepara el material, busca dentro del expediente, produce los Word y **conserva intactas sus notas del archivo de estado**. **Ningún comando exige Python para arrancar, y todos declaran cuándo no lo tuvieron.**

Si va a instalarlo: `python.org`, versión 3.9 o posterior, marcando «Add to PATH». Las bibliotecas las pide cada programa cuando le hacen falta, diciendo cuál — **y desde el 2026-09-21 eso es cierto en todos**: dos reventaban con un error de Python en pantalla, y se comprobó uno por uno. Todos los programas de hoy lo cumplen, y hay una prueba que falla el día que uno deje de cumplirlo.

### Las bibliotecas, y qué se pierde sin cada una

**No hace falta instalarlas todas, y ninguna hace falta para que los dieciséis comandos arranquen.** La tabla está aquí para que quien instala pueda decidir, en vez de descubrirlo cuando un comando lo diga:
...
| **faster-whisper · av · numpy** | `pip install faster-whisper av numpy` | **Pasar una grabación a texto** en esta misma máquina, sin que el audio salga de ella | `/transcribir-audio` **no transcribe nada y lo dice**, nombrando la biblioteca que falta. Junto con `/genoma-de-voz`, es lo único de esta tabla sin camino degradado —el modelo no puede oír ni calcular una huella de voz—: el resto del caso sigue, la grabación se declara y no se usa |
| **sherpa-onnx** | `pip install sherpa-onnx` | Que la transcripción **separe las voces** —«Hablante 1», «Hablante 2»— sin ponerles nombre, y que `/genoma-de-voz` saque la **huella de voz** de cada línea | La transcripción sale igual, **sin números de hablante y avisando de ello**. Se pierde saber si dos frases seguidas son de la misma persona. `/genoma-de-voz` **no prepara nada y lo dice** |
...
> **Lo que ninguna biblioteca cambia:** los dieciséis métodos, sus reglas y lo que se niegan a hacer. Python no añade criterio — **añade que el trabajo mecánico lo haga un programa** en vez de gastarse la lectura del modelo en él.
...
| **Que aparezcan los dieciséis**, no seis | Hasta el 2026-09-01 la documentación decía seis. Si salen menos de dieciséis, algo no se instaló |
...
| **Qué hace con un PDF escaneado sin texto** | Si no lo lee, no funciona ninguno de los dieciséis con material fotografiado. **Con Python, `/preparar-material` lo resuelve antes**: extrae el texto una vez y los demás trabajan sobre él. Conviene probarlo con un archivo real **antes** de sentarse a trabajar |
...
| Aparecen menos de dieciséis | Falta alguno de los `SKILL.md`. Se ve comparando con la lista del `README.md` |
