...
**Propósito.** Recorrer los documentos disponibles de un caso y producir tres cosas: una **tabla de anexos numerada** lista para pegar en un escrito; el **emparejamiento** entre cada documento y las afirmaciones que sirve para sostener; y un bloque separado con **lo que falta**, en sus tres clases distintas. Es trabajo mecánico y tedioso, y equivocarse cuesta caro: por eso se hace con método y no de memoria.
...
### Fase 1 — Recorrer todo antes de describir nada

Lista lo que hay en `1-Documentos recibidos/` (y en cualquier ruta que ella te señale): cuántos archivos, cómo se llaman, de qué tipo, cuántas páginas. **Todavía no describas nada.** Un archivo suele traer varios documentos (un escaneo con el contrato, dos comprobantes y un sobre) y un documento suele estar repartido en varios archivos. Quien describe archivo por archivo produce un inventario con la forma de la carpeta, no del caso.

- **Un archivo no es un anexo.** Un anexo es un documento. Anota los archivos que traen más de uno y los documentos partidos en varios.
- **Duplicados.** El mismo documento con dos nombres se reconoce comparando fecha, emisor y contenido, no el nombre. Se anexa una vez y se anota dónde más está.
- **Lo que no se pudo abrir o leer se declara.** Archivo dañado, formato que no puedes abrir, audio, página en blanco. Nunca en silencio.

### Fase 2 — Describir cada documento en una línea

Una línea por documento, con cinco datos: **qué es · quién lo produjo · de qué fecha · cuántas páginas · en qué archivo está**.

- **Qué es:** el nombre que el propio documento se da ("Contrato de arrendamiento", "Factura N.º 4471"). Si no se da ninguno, descríbelo por lo que se ve: *"Carta de una página, con membrete de X, dirigida a Y"*. Nunca lo llames por lo que crees que prueba.
...
- **La fecha es la que el documento muestra**, no la que trae el archivo en el computador: esa suele ser la del día en que se copió a la carpeta. Sin fecha impresa, se escribe **"sin fecha en el documento"** — nunca la del archivo, nunca una deducida. Y con la precisión que el documento permita y ni una más: si dice "marzo de 2024", eso se escribe.

> **Mal:** "Anexo 3 — Prueba del pago." → **Bien:** "Anexo 3 — Comprobante de transferencia — producido por el banco X — 14/03/2024 — 1 página — `transferencias.pdf`, página 2."
...
Para cada documento, recorre las afirmaciones y anota a cuáles sirve. Después **da la vuelta a la tabla** y comprueba, afirmación por afirmación, con qué documentos cuenta. Las dos direcciones se entregan: son la misma información, y cada una hace visible un vacío distinto.

**Reglas de la fase:**
- **Señala la parte, no el documento entero.** "El contrato" no es un emparejamiento; "cláusula cuarta, página 3" sí. Si no puedes señalar dónde, no puedes emparejar.
...
### Fase 5 — Detectar lo que falta: es la parte de mayor valor y tiene sección propia, **§5**

### Fase 6 — Numerar, revisar y entregar
...
**Cómo se detecta.** Recorriendo lo que el propio material nombra: un contrato que remite a su anexo 2; una carta que dice "adjunto copia de"; una factura que cita una orden de compra; una numeración que salta; una entrevista donde ella menciona un correo. **Cómo se escribe.** Con la **mención literal y dónde está**, y ni una palabra más: *"La cláusula sexta (página 4) remite a un «Anexo 2 — cronograma»; entre el material revisado no hay ningún documento con ese nombre ni con ese contenido."* **Prohibido dar por existente lo mencionado:** que un contrato nombre un anexo dice que el contrato lo nombra, no que el anexo exista ni que alguien lo tenga.

### 5.2 Clase B — afirmación sin ningún documento

**Cómo se detecta.** Solo con la tabla dada vuelta (Fase 4): son las afirmaciones cuya lista de anexos quedó vacía. Por eso las dos direcciones no son un adorno. **Cómo se escribe.** La afirmación, en su texto exacto; **sobre qué se apoya hoy** (normalmente el dicho de alguien, con dónde lo dice); y **qué clase de documento hablaría de eso**, descrito por su contenido: *"un documento que muestre la fecha del pago"*. **Prohibido** decir que la afirmación es débil, insuficiente o que no debería ir en el escrito: tú informas que hoy va sola, el resto es de ella.

### 5.3 Clase C — presente pero no utilizable

Seis formas, y siempre se dice **cuál** de ellas es, porque cada una se pide distinto:
...
| N.º | Qué es | Quién lo produjo | Fecha del documento | Págs. | Archivo | Sirve para |
|-----|--------|------------------|---------------------|-------|---------|------------|
| 1   | «…»    | «…»              | «…»                 | «…»   | «…»     | «H-01, H-04» |
...
4. OBSERVACIONES POR ANEXO  (solo los que tengan alguna)
   Anexo «N» — «parte ilegible, anotaciones a mano, desacuerdo entre el
   nombre del archivo y el documento, copia, autores distintos…»
...
| 3 | Captura de conversación      | Contenido: ambas partes / captura: la propia interesada | sin fecha en el documento | 2 | fotos/IMG_0431.jpg | H-02, H-05 |
```
...
**Sobre el recorrido y cada fila**
1. ¿Recorrí todo antes de describir, declaré lo que no pude abrir o leer, separé los archivos con varios documentos y uní los documentos partidos?
2. ¿**Todas** las filas dicen quién produjo el documento, incluidas las que produjo la propia interesada?
3. ¿Ese dato salió de lo que el documento muestra, y no del nombre del archivo ni de lo que alguien contó?
4. ¿Alguna fecha es la del archivo en el computador, en vez de la del documento? ¿Alguna es más precisa que la que el documento trae?
5. ¿Llamé a algún documento por lo que creo que prueba en vez de por lo que es?
