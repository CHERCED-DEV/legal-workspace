...
**Regla de escritura, dura.** Este método **no mueve, no renombra, no borra y no corrige ningún archivo**. Escribe en un solo sitio: `0-Estado del caso (no editar).txt`. Nunca escribe dentro de `1-Documentos recibidos/`: eso es el material tal como llegó y es lo único que no se puede reconstruir.

**No lo uses para:** valorar si el caso es fuerte o débil; pronosticar; recomendar qué hacer; calcular plazos; redactar ni completar borradores; construir los hechos con su prueba (eso es `fact-builder`).
...
2. **Responde la lista del §7 sobre tu propia salida.** Si algo falla, corrige; si no puedes, dilo en la entrega.
...
4. **Reescribe el archivo** según §4, conservando intacto lo que ella hubiera escrito, y **muestra en la salida el texto exacto que quedó guardado**, para que pueda desmentirlo de un vistazo. Nunca corrijas el resumen en silencio.
...
**Por qué dice "(no editar)".** Porque el sistema lo reescribe entero y ella perdería lo que escriba. Pero **si escribió algo, no se borra jamás**: se conserva palabra por palabra en el bloque de notas, que el sistema nunca toca.

**Cinco reglas:**
1. Se reescribe entero en cada pasada; el bloque de notas de ella se conserva intacto.
...
5. ¿Convertí un plazo mencionado en una fecha, o dije si corría o si venció?
...
10. ¿Moví, renombré, borré o completé algo? **No debí tocar nada fuera de `0-Estado del caso (no editar).txt`.**
11. ¿Mi salida deja claro, al principio y al final, que esto es lo que consta y no lo que ocurrió?
